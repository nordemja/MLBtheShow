...
getProfile = requests.get(
    "https://mlb21.theshow.com/universal_profiles/angry_platypus62/game_history?mode=ARENA",
    headers=headers,
)
...
with open(
    "C:\\Users\\justi\\OneDrive\\Documents\\Python Programming\\MLBtheShow\\rankedSeasonsStatsHITTER.csv",
    "w",
    newline="",
    encoding="utf-8",
) as csvFile:
    ...
    for x in range(1, totalPages + 1):
        while True:
            try:
                print("page: " + str(x))
                getProfile = requests.get(
                    "https://mlb21.theshow.com/universal_profiles/angry_platypus62/game_history?page="
                    + str(x)
                    + "&mode=ARENA",
                    headers=headers,
                )
            ...
            if (awayTeam != "CPU" or homeTeam != "CPU") and innings >= 7:
                ...
                while True:
                    try:
                        ...
                        summaryList = (
                            soup.find("div", {"class": "well"}).text.strip().split()
                        )
                            ...
                            statsFile.writerow(
                                {
                                    ...
                                    "Position": "TOTALS",
                                    "Player": player,
                                    "AB": int(AB) - int(pitcherAB),
                                    "R": int(R) - int(pitcherR),
                                    "H": int(H) - int(pitcherH),
                                    "RBI": int(RBI) - int(pitcherRBI),
                                    "BB": int(BB) - int(pitcherBB),
                                    "SO": int(SO) - int(pitcherSO),
                                    "AVG": AVG,
                                }
                            )
