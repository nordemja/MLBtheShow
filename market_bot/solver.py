...
def doSellOrders(currentHeaders, cardSeriesLink, browser):
    ...
    while True:
        try:
            completedPage = requests.get(
                base_path + "orders/completed_orders", headers=currentHeaders
            )
                ...
                try:
                    ...
                    if orderType == "Bought":
                        ...
                        sellPlayerLink = (
                            ...
                            + sellPlayerLink["href"].lstrip().rstrip()
                        )
                        sellableBefore = getTotalSellable(
                            sellPlayerLink, currentHeaders
                        )
                        if sellableBefore > 0:
        ...
        if order == "buy":
            ...
            while True:
                try:
                    ...
                    data = placeBuyOrder(
                        readyList[each]["URL"],
                        readyList[each]["buy amount"],
                        readyList[each]["form_token"],
                        readyList[each]["auth token"],
                        stubsBefore,
                        data,
                    )
                except:
